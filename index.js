...
const [blackAndWhite, black, pink] = farmAnimals.split(' ').filter(animal => {
  return animal === 'cow' || animal === 'horse' || animal === 'pig';
}).map(animal => {
  if (animal === 'cow') return 'blackAndWhite';
  if (animal === 'horse') return 'black';
  if (animal === 'pig') return 'pink';
});
